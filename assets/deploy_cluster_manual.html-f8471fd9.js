import{_ as n,X as t,Y as s,Z as d,a1 as e,$ as a,a0 as o,a3 as l,C as u}from"./framework-a5f096d1.js";const r={},c=l(`<div class="hint-container info"><p class="hint-container-title">相关信息</p><p>此文档旨在探索手动部署需要部署哪些组件，对熟悉 openGemini 集群工作原理有所帮助。建议<a href="./production_deployment_using_gemix">使用 gemix 部署</a>生产集群，</p></div><p>集群部署可以把openGemini的三个组件都部署在一个节点上，也可以把组件分散部署在多个节点上。</p><p>下载二进制包，参考<a href="../quick_start/get_started">安装部署</a></p><h2 id="部署伪集群" tabindex="-1"><a class="header-anchor" href="#部署伪集群" aria-hidden="true">#</a> 部署伪集群</h2><p>把openGemini集群的所有组件都部署在同一个节点上，这种集群部署方式我们称之为伪集群部署。<br> 目前社区提供了部署脚本install_cluster.sh。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token operator">&gt;</span> <span class="token function">sh</span> scripts/install_cluster.sh
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><div class="hint-container tip"><p class="hint-container-title">提示</p><p>执行该命令，在不修改配置文件的前提下，可直接在本地拉起一个openGemini集群，包括1个ts-sql、3个ts-meta和2个ts-store组件。但该集群仅在本地回环地址127.0.0.1上监听运行，可用于本地功能测试和学习，不能对外提供访问服务。</p></div><p>若要让集群监听本机IP，让外部节点可以访问，配置上相对要复杂一点，虽然可行，但在生产环境中不推荐。<br> 同样以部署1个ts-sql、3个ts-meta和2个ts-store组成的集群为例。</p><h3 id="分配端口" tabindex="-1"><a class="header-anchor" href="#分配端口" aria-hidden="true">#</a> 分配端口</h3><p>所有节点均监听本机IP地址，如192.168.0.1，所以所有组件之间不能使用相同的端口，需重新分配。可以做如下分配（参考）：</p><figure><img src="https://user-images.githubusercontent.com/49023462/200800373-65a3ac6c-f38d-46ed-86d6-8b8f21232d50.png" alt="4" tabindex="0" loading="lazy"><figcaption>4</figcaption></figure><h3 id="配置文件修改" tabindex="-1"><a class="header-anchor" href="#配置文件修改" aria-hidden="true">#</a> 配置文件修改</h3><p>openGemini的集群配置项都集中在openGemini.conf文件中，我们进行集群配置时，如果在同一个节点上只部署一个ts-meta、一个ts-sql和一个ts-store，或者合部其中两个或一个组件，不存在相同组件部署在同一个节点上的情况(因为没有IP和端口冲突)，则可以考虑在该节点上所有组件共用一个配置文件openGemini.conf</p><p>显然部署伪集群是不能共用一个，因为在同一个节点上有3个ts-meta，2个ts-store，因为IP会有冲突，所以我们建议为每个组件对应一个配置文件。具体做法如下：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&gt; cp –rf openGemini.conf sql.conf
...
members = [&quot;192.168.0.1:8010&quot;, &quot;192.168.0.1:8012&quot;, &quot;192.168.0.1:8013&quot;]
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>如果还需要新增ts-store,可按照该配置，更换一下端口，再拉起即可</strong></p><h2 id="拉起集群" tabindex="-1"><a class="header-anchor" href="#拉起集群" aria-hidden="true">#</a> 拉起集群</h2><p>编辑脚本</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&gt; cp –rf scripts/install_cluster.sh  scripts/cluster.sh
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h2 id="标准集群部署" tabindex="-1"><a class="header-anchor" href="#标准集群部署" aria-hidden="true">#</a> 标准集群部署</h2><p>目前还没有开发集群部署的自动化脚本，只能手动部署，欢迎感兴趣的童鞋到社区贡献!<br> openGemini集群部署如图所示，集群有ts-meta(3x), ts-sql(2x), ts-store(2x)：</p><figure><img src="https://user-images.githubusercontent.com/49023462/200800488-5683ecc2-e06b-4b65-a8ca-33b3bceaf6e4.jpg" alt="5" tabindex="0" loading="lazy"><figcaption>5</figcaption></figure><p>这种部署方式，不存在两个相同组件被部署在同一个节点之上，没有端口竞争，则可以让同一个节点上的所有组件共用一个配置文件。<br> 以Node1：192.168.0.1为例，仅需修改配置文件openGemini.conf如下内容：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>[common]
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div>`,55);function m(v,p){const i=u("RouterLink");return t(),s("div",null,[c,d("p",null,[e("客户端连接，参考 "),a(i,{to:"/zh/guide/quick_start/get_started.html#%E5%91%BD%E4%BB%A4%E8%A1%8C-ts-cli"},{default:o(()=>[e("ts-cli命令行")]),_:1})])])}const q=n(r,[["render",m],["__file","deploy_cluster_manual.html.vue"]]);export{q as default};
